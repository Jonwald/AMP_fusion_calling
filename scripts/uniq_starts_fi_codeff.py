...
def main():
    ...
    #with open("test_out", "w+") as out:
        ...
        #for path in ["S1568-00245-CP1Lib1_S23_Resampled_uniq.R1.fusions"]:
            fusions_file = str(path) + "/FusionInspector-inspect/finspector.FusionInspector.fusions.tsv"
            coding_eff_file = str(path) + "/FusionInspector-inspect/finspector.FusionInspector.fusions.abridged.tsv.annotated.coding_effect"
            ...
            with open(chimeric_reads_file, newline='') as chim_in:
                ...
                if len(results) > 0:
                    for k in range(0, (len(results))):
                        line=results[k]
                    #for line in results:
                        splitfrags = line[11].split(',')
                        splitfrags= [x.replace('/1', '') for x in splitfrags]
                        splitfrags= [x.replace('/2', '') for x in splitfrags]
                        splitfrags=list(set(splitfrags))
                        spanningfrags = line[12].split(',')
                        spanningfrags= [x.replace('/1', '') for x in spanningfrags]
                        spanningfrags= [x.replace('/2', '') for x in spanningfrags]
                        spanningfrags=list(set(spanningfrags))
